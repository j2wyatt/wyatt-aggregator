// 好心人的订阅地址：https://gist.githubusercontent.com/zoecsoulkey/66cbdd192cac7381a7380adc6baf134b/raw/proxies.yaml
//
module.exports.parse = async (raw, { axios, yaml, notify, console }, { name, url, interval, selected }) => {
  let res = raw
  // res = addSecName(res, { axios, yaml, notify, console }, { name, url, interval, selected })

  res = await changeRules(res, { axios, yaml, notify, console }, { name, url, interval, selected })
  // 每个网站最多五十个节点
  res = await groupNumNodes(res, { axios, yaml, notify, console }, { name, url, interval, selected })
  // 手动选择的分组里每个网站最多五个
  res = await removeExtraNodes(res, { axios, yaml, notify, console }, { name, url, interval, selected });
  // 按照网站建立分组
  res = await madeGroupBySite(res, { axios, yaml, notify, console }, { name, url, interval, selected });
  return res
}

// 每个网站保留最多五十个节点
async function groupNumNodes(raw, { axios, yaml, notify, console }, { name, url, interval, selected }){
...
// 给每个网站建立一个分组
async function madeGroupBySite(raw, { axios, yaml, notify, console }, { name, url, interval, selected }) {
...
// 每个网站保留十个节点
async function removeExtraNodes(raw, { axios, yaml, notify, console }, { name, url, interval, selected }) {
  let obj = yaml.parse(raw);
...
// 给节点名字加上域名
function addSecName(raw, { axios, yaml, notify, console }, { name, url, interval, selected }){
  let obj = yaml.parse(raw);
  // 提取关键词的函数
...
// 添加第三方规则
function changeRules(raw, { axios, yaml, notify, console }, { name, url, interval, selected }){
  const newRules = `
rules:
